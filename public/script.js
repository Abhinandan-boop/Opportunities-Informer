...
function setConnectButtonState(isAuthenticated) {
  if (!connectGmailBtn) return;
  if (isAuthenticated) {
    connectGmailBtn.innerHTML = `<span>✓</span> Sign Out`;
    connectGmailBtn.classList.add("connected");
  } else {
    connectGmailBtn.innerHTML = `<span>＋</span> Connect Gmail`;
    connectGmailBtn.classList.remove("connected");
  }
}
...
async function initAuthAndEmails() {
  ...
  setConnectButtonState(authenticated);
